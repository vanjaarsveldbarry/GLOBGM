...
def put_directory(session, local_path, irods_path, session_refresh_interval=30):
    ...
    progress_bar = tqdm(total=total_entries, desc="Uploading", unit="file")
    ...
    def recursive_put(session, local_path, irods_path):
        ...
        for entry in os.scandir(local_path):
            ...
            # if current_time - last_refresh_time > session_refresh_interval:
                # session.cleanup()  # Close the current session
                # session = setup_iRodsSession(env_config, password, ca_file)  # Recreate the session
    ...
    # Start the recursive upload with session refresh
