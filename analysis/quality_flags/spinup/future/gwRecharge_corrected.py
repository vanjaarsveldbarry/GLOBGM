...
data_path = Path('/projects/prjs1222/scratch_backup/globgm_scratch/analysis/quality_flags/data/spinup/cmip6_run/data')
yearly_savePath_temp = data_path / 'gwRecharge_annual.zarr'

#Nothing: 1:08 -- compute: 41 chunk:
cf_ds = xr.open_zarr(data_path / 'historical/gwRecharge_correction_factor.zarr', chunks='auto')['correction_factor'].rename({'lat':'latitude', 'lon':'longitude'})
...
def _read_recharge():
    ds_recharge1 = xr.open_zarr(data_path / 'historical/gwRecharge.zarr', chunks='auto')['gwRecharge']
    ds_recharge2 = xr.open_zarr(data_path / 'ssp370/gwRecharge.zarr', chunks='auto')['gwRecharge']
    ds_recharge =  xr.concat([ds_recharge1, ds_recharge2], dim='time').rename({'lat':'latitude', 'lon':'longitude'})
...
def _read_precip():
    ds_precip1 = xr.open_dataset(data_path / 'historical/precipitation.nc', chunks='auto')['precipitation']
    ds_precip2 = xr.open_dataset(data_path / 'ssp370/precipitation.nc', chunks='auto')['precipitation']
    ds_precip = xr.concat([ds_precip1, ds_precip2], dim='time').rename({'lat':'latitude', 'lon':'longitude'})
