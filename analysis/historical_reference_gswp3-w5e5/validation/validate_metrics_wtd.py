...
test = 'jarno'
dataDir = Path(f'/scratch-shared/_bvjaarsveld1/temp/validation/output/{test}')
...
def calculate_mean_bias(validation_df):
    mean_bias = validation_df.groupby('id_gerbil').agg({'solution': 'first',
                                                        'depthCat': 'first',
                                                        'lon': 'first',
                                                        'lat': 'first',
                                                        'obs_wtd': 'mean',
                                                        'sim_wtd': 'mean'})
    mean_bias = mean_bias.reset_index()[['solution', 'id_gerbil','depthCat', 'obs_wtd', 'sim_wtd']]
...
def calculate_kge_by_station(validation_df):
    ...
    kge_by_station = kge_by_station.merge(validation_df[['id_gerbil', 'solution', 'depthCat', 'lat', 'lon']].drop_duplicates(), on='id_gerbil', how='left')
    kge_by_station[['solution', 'id_gerbil','depthCat','lat','lon','KGE']]
