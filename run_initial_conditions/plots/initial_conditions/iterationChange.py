...
fig.tight_layout()
fig.savefig('/home/bvjaarsveld1/projects/workflow/GLOBGM/run_initial_conditions/plots/initial_conditions/_plots/absolute_change.png')
...
for layer in [1, 2]:
    for idx, solution in enumerate([1, 2, 3, 4]):
        ...
        df_with_pump = pd.read_csv(inputFolder / f'tr_with_pump/mf6_post/s0{solution}_hds_l{layer}_abs.csv').melt(var_name='Iteration', value_name='Value')
        df_with_pump['Iteration'] = df_no_pump['Iteration'].str.extract(r'(\d+)').astype(int)
        df_with_pump = df_no_pump.rename(columns={'Value': 'with_pump'})
        df_with_pump['Label'] = 'with_pump'
        ...
        combined_df = combined_df.drop(columns=['bias_ss', 'no_pump', 'with_pump'])	
        melted_df = combined_df.melt(id_vars=['Iteration', 'Index', 'Label'], value_vars=['bias'], value_name='Value')
        melted_df['Value'] = melted_df['Value'].diff().div(melted_df['Value'].shift().diff())
        col_index = 0 if layer == 1 else 1
        row_index = idx
        sns.lineplot(data=melted_df, x='Index', y='Value', marker='o', hue='Label', ax=axes[row_index * 2 + col_index])
        ...
        if col_index == 0:
            ...
            axes[row_index * 2 + col_index].set_ylabel('')
fig.tight_layout()
fig.savefig('/home/bvjaarsveld1/projects/workflow/GLOBGM/run_initial_conditions/plots/initial_conditions/_plots/relativeChange.png')