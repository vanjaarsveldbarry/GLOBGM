...
rule write_model_input_setup:
    ...
    params:
        ...
        iniStatesFolder="/scratch-shared/_bvjaarsveld1/output_initial_conditions/gswp3-w5e5/tr_no_pump/mf6_mod/mf6_mod_1/glob_tr/models/run_output_bin/_ini_hds/",
...
rule write_model_input_maps:
    ...
    shell:
        '''
        sbatch -o {params.slurm_log_file} {params.run_script} {MODELROOT_TR} {params.label} {params.solution} {params.runType} {DATA_DIR} {output.outFile} {params.iniFileName}
        while [ ! -e {output.outFile} ]; do
            sleep 10
        done
        '''
...
rule modify_ini_conditions:
    ...
    output:
        outFile=f"{SLURMDIR_TR}/2_write_model_input/_modify_ini_conditions_complete"

    params:
        run_script=f"{RUN_GLOBGM_DIR}//model_tools_src/python/initial_states/initial_statesModifyPath.py",
    shell:
        '''
        cp -r /scratch-shared/_bvjaarsveld1/output_initial_conditions/gswp3-w5e5/tr_no_pump/mf6_mod/mf6_mod_1/glob_tr/models/run_output_bin/_ini_hds /scratch-shared/_bvjaarsveld1/output_initial_conditions/gswp3-w5e5/tr_with_pump/mf6_mod/mf6_mod_1/glob_tr/models/run_output_bin && python {params.run_script} {MODELROOT_TR}/mf6_mod/mf6_mod_1 && touch {output.outFile}
        '''
...
rule post_model_solution3:
    ...
    params:
        ...
        run_script=f"{RUN_GLOBGM_DIR}/model_job_scripts/4_post-processing/transient/_zarrWriter_with_pump.slurm",
...
rule analyse_heads:
    ...
    output:
        outFile=f"{SLURMDIR_TR}/4_post-processing/_analyse_heads_complete"
    params:
        run_script=f"{RUN_GLOBGM_DIR}/model_job_scripts/5_analyse/_pumping_ini/validation_obs.sh",
        modDir=f"{MODELROOT_TR}", 
        slurm_log_file=f"{SLURMDIR_TR}/4_post-processing/_analyse_heads.out",
        py_script=f"{RUN_GLOBGM_DIR}/model_job_scripts/5_analyse/_pumping_ini/scripts/validation_globgm.py",
        obs_file=f"{RUN_GLOBGM_DIR}/model_job_scripts/5_analyse/_pumping_ini/data/observed_gwh_for_ss_val_1960.gpkg",


    shell:
        '''
        sbatch -o {params.slurm_log_file} {params.run_script} {params.modDir} {output.outFile} {params.py_script} {params.obs_file}
        while [ ! -e {output.outFile} ] ; do
            sleep 10
        done
        '''
...
use rule run_model_solution3 as run_model_solution3_rep with:
    input:
        rules.analyse_heads.output.outFile,
    output:
...
use rule analyse_heads as analyse_heads_rep with:
    input:
    ...
    output:
        outFile=f"{SLURMDIR_TR}/4_post-processing/_analyse_heads_complete_rep"
    params:
        run_script=f"{RUN_GLOBGM_DIR}/model_job_scripts/5_analyse/_pumping_ini/validation_obs.sh",
        slurm_log_file=f"{SLURMDIR_TR}/4_post-processing/_analyse_heads_rep.out",
        modDir=f"{MODELROOT_TR}", 
        py_script=f"{RUN_GLOBGM_DIR}/model_job_scripts/5_analyse/_pumping_ini/scripts/validation_globgm.py",
        obs_file=f"{RUN_GLOBGM_DIR}/model_job_scripts/5_analyse/_pumping_ini/data/observed_gwh_for_ss_val_1960.gpkg",
...
rule wrap_up:
    input:
        ...
        rules.analyse_heads_rep.output.outFile,
    output:
