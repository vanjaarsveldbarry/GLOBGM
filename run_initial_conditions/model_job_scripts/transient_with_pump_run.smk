...
rule all:
    input:
        f"{SLURMDIR_TR}/4_post-processing/done_validation"
...
rule modify_ini_conditions:
    ...
    output:
        outFile=f"{SLURMDIR_TR}/2_write_model_input/done_modify_ini_conditions"

    params:
        run_script=f"{RUN_GLOBGM_DIR}/model_tools_src/python/initial_states/initial_statesModifyPath.py",
    shell:
        '''
        cp -r {OUTPUTDIRECTORY}/{SIMULATION}/tr_no_pump/mf6_mod/mf6_mod_1/glob_tr/models/run_output_bin/_ini_hds {OUTPUTDIRECTORY}/{SIMULATION}/tr_with_pump/mf6_mod/mf6_mod_1/glob_tr/models/run_output_bin && python {params.run_script} {MODELROOT_TR}/mf6_mod/mf6_mod_1 && touch {output.outFile}
        '''
